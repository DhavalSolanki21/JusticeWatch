...
from .serializers import UserRegistrationSerializer, CustomTokenObtainPairSerializer, UserProfileSerializer, ProfileUpdateSerializer
...
class ApproveLawyerView(APIView):
    ...
    def post(self, request, pk):
        try:
            ...
            lawyer.save()
            return Response({"message": f"Lawyer {lawyer.full_name} approved successfully."}, status=status.HTTP_200_OK)
        except User.DoesNotExist:
            return Response({"error": "Pending lawyer not found."}, status=status.HTTP_404_NOT_FOUND)
