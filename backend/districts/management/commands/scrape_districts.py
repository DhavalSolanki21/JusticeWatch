...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        self.stdout.write(
            self.style.NOTICE("Starting Web Scraping for States and Districts...")
        )
        ...
        for table in tables:
            ...
            while prev and not (
                prev.name in ["h2", "h3", "h4"]
                or (prev.name == "div" and "mw-heading" in prev.get("class", []))
            ):
                ...
                if dist_idx != -1:
                    for row in rows[1:]:
                        ...
                        if len(cols) > dist_idx:
                            ...
                            counter = 1
                            while (
                                District.objects.filter(code=dist_code).exists()
                                and not District.objects.filter(
                                    name=district_name
                                ).exists()
                            ):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully scraped and ingested {state_count} new States and {district_count} new Districts."
            )
        )
