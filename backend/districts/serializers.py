...
class DistrictSummarySerializer(serializers.ModelSerializer):
    ...
    def get_crime_distribution(self, obj):
        ...
        crime_counts = (
            Case.objects.filter(district=obj.district, case_category="Criminal")
            .values("crime_type")
    ...
    def get_chargesheet_distribution(self, obj):
        ...
        chargesheet_counts = (
            Case.objects.filter(district=obj.district)
            .values("chargesheet_status")
            .annotate(count=Count("id"))
        )
        return [
            {"status": item["chargesheet_status"] or "Pending", "count": item["count"]}
            for item in chargesheet_counts
        ]
