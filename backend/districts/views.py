...
class DistrictSummaryListView(APIView):
    ...
    def get(self, request):
        ...
        case_stats = Case.objects.values('district_id', 'case_status').annotate(count=Count('id'))
        ...
        for case_info in Case.objects.values('district_id', 'filed_date', 'disposed_date'):
        ...
        crimes = Case.objects.filter(case_category='Criminal').values('district_id', 'crime_type').annotate(count=Count('id')).order_by('district_id', '-count')
        crime_dict = {}
        for item in crimes:
            dist_id = item['district_id']
            ...
            if len(crime_dict[dist_id]) < 5:
                crime_dict[dist_id].append({'crime': item['crime_type'] or 'Other', 'count': item['count']})
                
        # 5. Prefetch chargesheet distribution
        chargesheets = Case.objects.values('district_id', 'chargesheet_status').annotate(count=Count('id'))
        chargesheet_dict = {}
        for item in chargesheets:
            dist_id = item['district_id']
            if dist_id not in chargesheet_dict:
                chargesheet_dict[dist_id] = []
            chargesheet_dict[dist_id].append({'status': item['chargesheet_status'] or 'Pending', 'count': item['count']})
            
        # 6. Prefetch category distribution
        categories = Case.objects.values('district_id', 'case_category').annotate(count=Count('id'))
        category_dict = {}
        for item in categories:
            dist_id = item['district_id']
            if dist_id not in category_dict:
                category_dict[dist_id] = []
            category_dict[dist_id].append({'category': item['case_category'] or 'Civil', 'count': item['count']})
