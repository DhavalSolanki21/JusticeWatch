...
class AnalyticsTests(APITestCase):
    def setUp(self):
        ...
        self.district = District.objects.create(state=self.state, name="Ahmedabad", code="AHM")
    ...
    def test_predictions_overview(self, mock_predict):
        ...
        self.assertEqual(response.data['duration_distribution']['Long-Pending (>2yr)'], 2) # Both returned 'high'
    ...
    def test_advanced_predict_case_id(self, mock_predict):
        ...
        self.client.force_authenticate(user=self.lawyer)
        response = self.client.post(reverse('analytics-predict-custom'), {'case_id': self.case1.id})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
