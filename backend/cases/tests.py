...
class CasesTests(APITestCase):
    def setUp(self):
        ...
        self.district_ahm = District.objects.create(state=self.state, name="Ahmedabad", code="AHM", population=1000000)
        self.district_sur = District.objects.create(state=self.state, name="Surat", code="SUR", population=800000)
        ...
        CaseAssignment.objects.create(case=self.case_ahm, lawyer=self.lawyer1, representing="Petitioner")
    ...
    def test_lawyer_update_unassigned_case(self):
        ...
        self.client.force_authenticate(user=self.lawyer1)
        detail_url = reverse('case-detail', args=[self.case_sur.id]) # lawyer1 not assigned
