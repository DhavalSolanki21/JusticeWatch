...
def fetch_data(sample_size=None):
    ...
    df = df.replace(
        [-9999, -9998, "-9999", "-9998", "-9999 missing name", "-9998 unclear"], np.nan
    )
    ...
    df["filing_to_first_list_days"] = (
        df["date_first_list"] - df["date_of_filing"]
    ).dt.days
    df["filing_to_first_list_days"] = df["filing_to_first_list_days"].fillna(
        30
    )  
    df["filing_to_first_list_days"] = np.where(
        df["filing_to_first_list_days"] < 0, 30, df["filing_to_first_list_days"]
    )

    df["listing_gap_days"] = (df["date_last_list"] - df["date_first_list"]).dt.days
    df["listing_gap_days"] = df["listing_gap_days"].fillna(180)  
    df["listing_gap_days"] = np.where(
        df["listing_gap_days"] < 0, 180, df["listing_gap_days"]
    )

    df["court_caseload"] = df.groupby(["state_code", "dist_code", "court_no"])[
        "ddl_case_id"
    ].transform("count")
    ...
    df = df.merge(
        type_key_unique[["type_name", "type_name_s"]], on="type_name", how="left"
    )

    disp_key_unique = disp_key.drop_duplicates("disp_name")
    df = df.merge(
        disp_key_unique[["disp_name", "disp_name_s"]], on="disp_name", how="left"
    )
    ...
    def bin_disposal(disp_s):
        ...
        if disp_s in ["convicted", "plead guilty"]:
            return "Convicted / Guilty"
        if disp_s in ["acquitted", "quash"]:
            return "Acquitted / Quashed"
        if disp_s in ["dismissed", "reject"]:
            return "Dismissed / Rejected"
        if disp_s in ["compromise", "referred to lok adalat", "withdrawn"]:
            return "Settled / Compromised"
        if disp_s in ["transferred", "remanded", "committed"]:
            return "Transferred / Remanded"
        if disp_s in ["allowed", "judgement", "ex-parte"]:
            return "Judgement (Merits)"
        if disp_s == "abated":
            return "Abated"
...
def train_models(df=None):
    ...
    try:
        ...
        fig = px.line(
            ...
            x="filing_month",
            y="case_count",
            title="Case Filings Trend Over Time",
        )
    ...
    print(
        f"Polynomial Regression -> MAE: {mae_poly:.2f}, MSE: {mse_poly:.2f}, R2: {r2_poly:.4f}"
    )
    ...
    print(
        f"Random Forest -> Accuracy: {acc_rf:.4f}, Macro Sensitivity: {rf_sens:.4f}, Macro Specificity: {rf_spec:.4f}"
    )
    print(
        f"Decision Tree -> Accuracy: {acc_dt:.4f}, Macro Sensitivity: {dt_sens:.4f}, Macro Specificity: {dt_spec:.4f}"
    )
    print(
        f"K-Nearest Neighbors -> Accuracy: {acc_knn:.4f}, Macro Sensitivity: {knn_sens:.4f}, Macro Specificity: {knn_spec:.4f}"
    )
    ...
    mae_male_def = (
        mean_absolute_error(
            male_subset["actual_duration"], male_subset["predicted_duration"]
        )
        if len(male_subset) > 0
        else 0
    )
    mae_fem_def = (
        mean_absolute_error(
            fem_subset["actual_duration"], fem_subset["predicted_duration"]
        )
        if len(fem_subset) > 0
        else 0
    )
    ...
    print(
        f"Average Error (Male Defendant): {mae_male_def:.2f} days (N={len(male_subset)})"
    )
    print(
        f"Average Error (Female Defendant): {mae_fem_def:.2f} days (N={len(fem_subset)})"
    )
