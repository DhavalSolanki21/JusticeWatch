...
def run_ingestion(sample_size=1000):
    ...
    cases_iter = pd.read_csv(os.path.join(base_path, 'cases', 'cases_2018.csv'), chunksize=100000, low_memory=False)
    ...
    df = df.merge(type_key_2018[['type_name', 'type_name_s']], on='type_name', how='left')
    
    disp_key_2018 = disp_key[disp_key['year'] == 2018].drop_duplicates('disp_name')
    df = df.merge(disp_key_2018[['disp_name', 'disp_name_s']], on='disp_name', how='left')
    
    purpose_key_2018 = purpose_key[purpose_key['year'] == 2018].drop_duplicates('purpose_name')
    df = df.merge(purpose_key_2018[['purpose_name', 'purpose_name_s']], on='purpose_name', how='left')
    
    dist_key_2018 = dist_key[(dist_key['year'] == 2018) & (dist_key['state_code'] == 17)].drop_duplicates('dist_code')
    df = df.merge(dist_key_2018[['dist_code', 'district_name']], on='dist_code', how='left')
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        ...
        dist_name = row['district_name'] if pd.notna(row['district_name']) else f"District {row['dist_code']}"
        district_obj, _ = District.objects.get_or_create(
            name=dist_name,
            defaults={'state': state_obj, 'code': str(row['dist_code'])[:10], 'population': 1000000}
        )
        ...
        case_obj = Case.objects.create(
            ...
            court_name=f"Court No {row['court_no']}" if pd.notna(row['court_no']) else "District Court",
            case_category=case_category,
            crime_type=str(row['type_name_s'])[:99] if pd.notna(row['type_name_s']) else "Other",
            applicable_sections="Multiple", # We could use acts_sections.csv, but keep it simple for now
            filed_date=row['date_of_filing'].date(),
            disposed_date=row['date_of_decision'].date() if pd.notna(row['date_of_decision']) else None,
            case_status=status,
            num_parties=2,
        ...
        if pd.notna(row['date_last_list']):
            Hearing.objects.create(
                ...
                purpose=str(row['purpose_name_s'])[:254] if pd.notna(row['purpose_name_s']) else "Routine Hearing",
                next_hearing_date=row['date_next_list'].date() if pd.notna(row['date_next_list']) else None,
