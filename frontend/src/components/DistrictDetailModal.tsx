...
interface DistrictDetailModalProps {
  ...
  onClose: () => void;
  onViewAllCases: (districtName: string) => void;
}

export default function DistrictDetailModal({ district, onClose, onViewAllCases }: DistrictDetailModalProps) {
  ...
  if (district.category_distribution) {
    district.category_distribution.forEach(item => {
      ...
      if (lower.includes('civil')) civilCount += item.count;
      else if (lower.includes('criminal')) criminalCount += item.count;
      else if (lower.includes('appeal')) appealCount += item.count;
      else civilCount += item.count; // Default unknown to civil
    });
  }
