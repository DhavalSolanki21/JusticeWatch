...
interface DistrictGeoShape {
  ...
  labelX: number;
  labelY: number;
  region: "Kutch" | "Saurashtra" | "North" | "Central" | "South";
}

const DISTRICT_SHAPES: DistrictGeoShape[] = [
  // 1. KUTCH (North-West)
...
const getSeverityFill = (severity: string, isHovered: boolean) => {
  switch (severity) {
...
const matchDistrictId = (geojsonName: string): string => {
...
export default function StateMap({ districts = [], onDistrictClick, stateCode = "GJ" }: StateMapProps) {
  const [hoveredDistrictId, setHoveredDistrictId] = useState<string | null>(null);

  const getDistrictDetails = (id: string): DistrictSummary | undefined => {
